...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    ...
    if s:
        if install and AUTOINSTALL:  # check environment variable
            ...
            LOGGER.info(f"{prefix} ultralytics_MB requirement{'s' * (n > 1)} {pkgs} not found, attempting AutoUpdate...")
            try:
                t = time.time()
        ...
        if "u.yaml" in file:
            ...
            if file != original_file and verbose:
                LOGGER.info(
                    ...
                    f"trained with https://github.com/ultralytics/ultralytics_MB and feature improved performance vs "
...
def collect_system_info():
    ...
    from ultralytics_MB.utils import ENVIRONMENT  # scope to avoid circular import
    from ultralytics_MB.utils.torch_utils import get_cpu_info, get_gpu_info
...
def check_amp(model):
    ...
    try:
        ...
        LOGGER.warning(
            ...
            f"Unable to load YOLO11n due to possible ultralytics_MB package modifications. {warning_msg}"
        )
...
def print_args(args: Optional[dict] = None, show_file=True, show_func=False):
    ...
    def strip_auth(v):
        """Clean longer ultralytics_MB HUB URLs by stripping potential authentication information."""
