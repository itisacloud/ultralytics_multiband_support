...
def get_cpu_info():
    ...
    from ultralytics_MB.utils import PERSISTENT_CACHE  # avoid circular import error
...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    s = f"ultralytics_MB {__version__} 🚀 Python-{PYTHON_VERSION} torch-{torch.__version__} "
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    try:
        ...
        LOGGER.warning(f"WARNING ⚠️ Skipping {f}, not a valid ultralytics_MB model: {e}")
        return {}
...
def profile(input, ops, n=10, device=None):
    """
    ultralytics_MB speed, memory and FLOPs profiler.
